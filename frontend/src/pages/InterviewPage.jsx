...
const InterviewPage = () => {
  ...
  const [modalOpen, setModalOpen] = useState(true); // Modal başlangıçta açık
  const [consentGiven, setConsentGiven] = useState(false); // Checkbox durumu

  useEffect(() => {
    // Sayfa yüklendiğinde modal'ı aç
    setModalOpen(true);
  }, []);
  ...
  const checkPermissions = async () => {
    try {
      ...
      console.error('Kamera veya mikrofon izni reddedildi:', error);
      return false;
    }
  };
  ...
  const startInterview = async () => {
    ...
    setInterviewStarted(true); // Mülakat başladı
    setInterviewEnded(false); // Mülakat bitiş durumu sıfırlandı
  };
  ...
  const handleModalSubmit = () => {
    if (consentGiven) {
      setModalOpen(false); // Modal kapat
